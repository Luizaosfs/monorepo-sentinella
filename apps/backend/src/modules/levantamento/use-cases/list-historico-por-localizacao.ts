...
@Injectable()
export class ListHistoricoPorLocalizacao {
  constructor(private prisma: PrismaService) {}

  execute(clienteId: string, lat: number, lng: number, tolerance: number) {
    ...
    return this.prisma.client.$queryRaw(Prisma.sql`
      ...
      WHERE li.cliente_id = ${clienteId}::uuid
        ...
        AND li.latitude  BETWEEN ${lat - tol}  AND ${lat + tol}
        AND li.longitude BETWEEN ${lng - tol}  AND ${lng + tol}
