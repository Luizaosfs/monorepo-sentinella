-- Recria v_focos_risco_todos alinhando com v_focos_risco_ativos (post-SLA inteligente).
-- O fr.* expandido na versão anterior (20260746000000) conflitava com colunas
-- adicionadas depois (sla_status, score_prioridade, status_sla_inteligente etc.),
-- causando retorno vazio em alguns clientes PostgreSQL.
...
SELECT
  ...
  -- Dados mínimos inline (igual a v_focos_risco_ativos)
  (
...
COMMENT ON VIEW v_focos_risco_todos IS
  'Todos os focos incluindo resolvido/descartado. '
  'Mesma estrutura de v_focos_risco_ativos sem filtro de status. '
  'Recriada em 20270101000011 para corrigir conflito de colunas com migrations de SLA inteligente. '
